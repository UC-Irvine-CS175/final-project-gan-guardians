...
class BPSDataModule(pl.LightningDataModule):
    ...
    def prepare_data(self) -> None:
        ...
        # train_csv file.
        train_path = os.path.join(self.train_dir, self.train_csv)
        ...
        # call the save_tiffs_local_from_s3 function to download tiffs from the
        # val_csv file.        
        val_path = os.path.join(self.val_dir, self.val_csv)
        save_tiffs_local_from_s3(self.s3_client, 
                                 self.bucket_name,
                                 self.s3_path,
                                 val_path, 
        ...
        if self.test_csv is not None:
            ...
            test_path = os.path.join(self.test_dir, self.test_csv)
            save_tiffs_local_from_s3(self.s3_client,
                                     self.bucket_name,
                                     self.s3_path,
                                     test_path, 
    ...
    def setup(self, stage: str):
        ...
        if stage == "train":
            ...
            self.train_BPSMouseDataset = BPSMouseDataset(meta_csv_file = self.train_csv,
        ...
        if stage == "validate":
            ...
            self.validate_BPSMouseDataset = BPSMouseDataset(meta_csv_file = self.val_csv,
        ...
        if stage == "test":
            ...
            self.test_BPSMouseDataset = BPSMouseDataset(meta_csv_file = self.test_csv,
    ...
    def val_dataloader(self) -> EVAL_DATALOADERS:
        ...
        """
        return DataLoader(self.validate_BPSMouseDataset, batch_size=self.batch_size, num_workers=self.num_workers)
    ...
    def test_dataloader(self) -> EVAL_DATALOADERS:
        ...
        """
        return DataLoader(self.test_BPSMouseDataset, batch_size=self.batch_size, num_workers=self.num_workers)
