...
def preprocess_images(lt_datamodule: DataLoader):
    """
...
def perform_tsne(X_reduced_dim: np.ndarray,
    ...
    tsne_obj = TSNE(n_components=n_components, 
                    ...
                    angle=angle, 
                    verbose=verbose).fit_transform(X_reduced_dim)

    # Fit the TSNE object to X_reduced_dim
    # tsne_obj.fit(X_reduced_dim)
...
def create_tsne_cp_df(X_tsne: np.ndarray,
    ...
    cps_df = pd.DataFrame(columns=df_cols, data=np.column_stack((X_tsne[:num_points], target.iloc[:num_points])))
    
    # Convert the target column to an integer type
    # pd.to_numeric(cps_df["target_series"], downcast="integer")
    ...
    # cps_df["particle_type"] = particle_type
    cps_df["particle_type"] = cps_df.apply(lambda x: particle_type[x["target"]], axis=1)
